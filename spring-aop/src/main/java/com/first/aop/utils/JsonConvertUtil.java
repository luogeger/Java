...
public class JsonConvertUtil {
    ...
    /**
     * ResultBody.getData()为数组时, 转化为List<Bean>
     * @param source
     * @param clazz
     * @param <T>
     * @return
     */
    public static <T> List<T> parseListAsBean(Object source, Class<T> clazz) {
        JSONArray jsonArray = parseJSONArray(source);
        List<T> resultList = arrayShiftToBean(jsonArray, clazz);
        return resultList;
    }
        ...
        array.stream()
                ...
                .forEach(item -> {
                    try {
                        ...
                        logger.info("JSONArray转换失败!\n", e);
                        throw new IllegalArgumentException("JSONArray转换失败!");
                    }
                });
        return resultList;
    }
